<script lang="ts">
  ...
  import { fetchAttendance, generateDates, downloadCsv } from './lib/api'
  ...
  const totalPresent = $derived(
    filteredUsers.reduce((sum, u) => sum + u.totalPresent, 0),
  )
  const totalAbsent = $derived(
    filteredUsers.reduce((sum, u) => sum + u.totalAbsent, 0),
  )
