import type { ExportData, UserAttendance, AttendanceEntry } from './types'
...
function parseUsername(
  ...
  if (commaIdx > -1) {
    return {
      surname: username.substring(0, commaIdx).trim(),
      givenName: username.substring(commaIdx + 1).trim(),
    }
  }
  const spaceIdx = username.indexOf(' ')
  if (spaceIdx > -1) {
    return {
      surname: username.substring(0, spaceIdx).trim(),
      givenName: username.substring(spaceIdx + 1).trim(),
    }
  }
...
export async function fetchAttendance(
  ...
  const res = await fetch(`${API_BASE}/api/export/data?${params}`)
  ...
  for (const r of data.records) {
    ...
    const timeOut = r.timeOut
      ? new Date(r.timeOut).toLocaleTimeString('en-US', {
          hour: '2-digit',
          minute: '2-digit',
          hour12: false,
        })
...
export function downloadCsv(from?: string, to?: string, type: string = 'employee') {
  const range = getWeekRange()
  const fromDate = from || range.from
  const toDate = to || range.to
  const params = new URLSearchParams({ from: fromDate, to: toDate, type })
